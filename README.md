...
- 실행 결과를 입력 받는 처리를 추가한다. 각각의 결과값은 플레이어name 입력과 마찬가지로 (",") 쉼표로 구분한다.
- Ladder클래스에 실행 결과를 저장할 List를 선언하고 `List<resultTag>` 로 결과를 관리한다.  
  - 생성자의 매개변수에 결과값을 추가로 받도록 설정한다.  
  - resultTag 클래스를 만들고 결과를 저장한다. resultTag 사이즈는 nameTag 사이즈와 동일하게 설정.
- 사다리 모형의 마지막 줄에 결과 목록도 나타나도록 출력을 수정한다.
- 결과를 확인할 플레이어name을 입력 받는 처리를 추가한다.
